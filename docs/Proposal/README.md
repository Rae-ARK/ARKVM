...
| File | Covers |
| --- | --- |
| [`ARKVM-EXECUTION-MODEL-PROPOSAL.md`](ARKVM-EXECUTION-MODEL-PROPOSAL.md) | What ARKVM executes and owns. Three candidate shapes (renderer only; renderer plus reactive core; general VM). How ARKVM squares with ARKlight's "compiler first, runtime last" agreement. The embedder boundary and load-time verification. **Proposed, not accepted.** |
